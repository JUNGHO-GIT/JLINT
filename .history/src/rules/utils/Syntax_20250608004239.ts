...
export const capitalize = async (
  ...
  try {
    ...
    const rules1 = (
      /([^</>\s]\b)(select|from|where|insert|into|values|update|set|delete|join|on|create)([^</>]\b)/gm
    );
    const rules2 = (
      /([^</>]\b)(\s*)(table|alter|drop|truncate|order by|group\s+by|having|limit|offset|distinct)(\s*)([^</>]\b)/gm
    );
    const rules3 = (
      /([^</>]\b)(\s*)(and|or|not|null|is|in|like|between|as)(\s*)([^</>]\b)/gm
    );
