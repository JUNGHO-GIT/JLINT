...
export const capitalize = async (
  ...
  try {
    ...
    const rules1 = (
      /([^</>]\b)(\s*)(select|from|where|insert|into|values|update|set|delete|on|create)(\s*)(?!<[^</>])/gm
    );
    const rules2 = (
      /([^</>]\b)(\s*)(table|alter|drop|truncate|order by|group by|having|limit|offset|distinct)(\s*)(?!<[^</>])/gm
    );
    const rules3 = (
      /([^</>]\b)(\s*)(or|not|ifnull|is|in|like|between|as|case when|then|date_format)(\s*)(?!<[^</>])/gm
    );
    const rules4 = (
      /([^</>]\b)(\s*)(inner join|left join|right join|full join|outer join)(\s*)(?!<[^</>])/gm
    );
