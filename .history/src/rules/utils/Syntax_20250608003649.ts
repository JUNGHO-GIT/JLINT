...
export const capitalize = async (
  ...
  try {
    ...
    const rules1 = (
      /(\s*)([^</>]\b)(select|from|where|insert|into|values|update|set|delete|join|on|create)([^</>]\b)(\s*)/gm
    );
    const rules2 = (
      /(\s*)([^</>]\b)(table|alter|drop|truncate|order by|group\s+by|having|limit|offset|distinct)([^</>]\b)(\s*)/gm
    );
    const rules3 = (
      /(\s*)([^</>]\b)(and|or|not|null|is|in|like|between|as)([^</>]\b)(\s*)/gm
    );

    const result = lodash.chain(contentsParam)
    .replace(rules1, (_, p1, p2, p3, p4, p5) => (
      `${p1}${p3.toUpperCase()}${p5}`
    ))
