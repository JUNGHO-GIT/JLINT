...
const dynamicImport = async (specifier: string) => {
  ...
  if (useEsm) {
    try {
      ...
      logger(`error`, `dynamicImport - ESM import failed for ${specifier}: ${(error as Error).message}`);
      return null;
    }
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger(`error`, `dynamicImport - all attempts failed for ${specifier}: ${(error as Error).message}`);
        return null;
      }
    }
  }
