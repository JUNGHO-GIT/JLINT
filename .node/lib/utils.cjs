...
const logger = (type=``, value=``) => {
	const config = {
		...
		"info": { "str": `[INFO]`, "color": `\u001b[38;5;46m` },
		"success": { "str": `[SUCCESS]`, "color": `\u001b[38;5;82m` },
...
const spawnWrapper = (cmd=``, args=[]) => {
	...
	const result = spawnSync(cmd, args, { "encoding": `utf8`, "env": newEnv });
	return result;
};
...
const getProjectType = (args=``) => {
	...
	const result = { isClient, isServer, hasVite, hasNext, hasReactScripts, hasIndexTs };
	return result;
};
