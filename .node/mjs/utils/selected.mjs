...
import { T } from "../ps/lib/classes.mjs";
import { runScript } from "../ps/lib/script-runtime.mjs";
...
const printScriptList = (scriptList = []) => {
  T.PrintLine(`Yellow`);
  T.PrintText(`Yellow`, `▶ 실행 가능한 MJS 파일 목록`);
  scriptList.forEach((script) => {
    T.PrintText(`Cyan`, `  ${script.number}. ${script.name}`);
  });
};
...
const executeScript = async () => {
  ...
  if (scriptList.length === 0) {
    T.PrintText(`Red`, `! 실행 가능한 mjs 파일이 없습니다.`);
    process.exit(1);
  }
  ...
  T.PrintLine(`Yellow`);
  const input = await T.TextInput(`Green`, `실행할 번호를 입력하세요 (1-${scriptList.length}):`);
  const selectedScript = getSelectedScript(input, scriptList);

  if (!selectedScript) {
    T.PrintText(`Red`, `! 유효한 번호를 입력하세요.`);
    process.exit(1);
  }

  T.PrintLine(`Yellow`);
  T.PrintText(`Green`, `▶ 선택한 스크립트 실행: ${selectedScript.name}`);
  await runSelectedScript(selectedScript);
  T.PrintLine(`Green`);
  T.PrintText(`Green`, `✓ 실행 완료: ${selectedScript.name}`);
};
