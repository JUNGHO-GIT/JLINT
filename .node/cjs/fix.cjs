...
const runTsPrune = () => {
	...
	for (const mth of exeMths) {
		...
		const [cmd, mthArgs] = mth.getCommand(cmdPath);
...
const removeNamesInExportDeclarations = (sf, tgtNames) => {
	...
	for (const expDecl of expDecls) {
		...
		for (const spec of specs) {
			...
			if (tgtNames.has(expName) || tgtNames.has(localName)) {
				toRm.push(spec);
			}
		}
...
const processFile = (proj, fp='', names) => {
	...
	return { "file": fp, "removed": Array.from(rmSet), "skipped": false, "reason": '' };
};
...
const runFixProcess = () => {
	...
	for (const [file, items] of grpByFile.entries()) {
